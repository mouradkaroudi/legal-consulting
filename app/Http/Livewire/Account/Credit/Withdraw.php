...
namespace App\Http\Livewire\Account\Credit;
...
class Withdraw extends Component implements HasForms
{
	...
	protected function getFormSchema(): array
	{
		...
		$userWithdrawalMethods = $this->user->withdrawal_methods ?? [];

		$userWithdrawalMethodsIds = array_column($userWithdrawalMethods, 'method_id');

		$withdrawalMethods = WithdrawalMethod::whereIn('id', $userWithdrawalMethodsIds)->get();
	...
	{
		return view("livewire.account.credit.withdraw");
	}
	...
	public function submit()
	{
		...
		try {
			TransactionService::withdraw($this->user, [
				'amount' => $this->amount,
			...
			redirect()->route("account.credit.index");
		} catch (\Throwable $th) {
