@php
    $txn = \App\Models\Transaction::find($notification->data['txn_id']);
@endphp
...
<div class="flex w-full">
        ...
        <div class="mr-3 text-sm font-normal">
            <span class="mb-1 text-sm font-semibold text-gray-900 dark:text-white">
                {{ __('notifications.transactions.' . $notification->data['status'] . '.title' ) }}
            </span>
            <div class="mb-2 text-sm font-normal">
                {{ __('notifications.transactions.' . $notification->data['status'] . '.body', ['amount' => $txn->formattedAmount, 'txn_id' => $txn->id, 'body' => $notification->data['body']] ) }}
            </div> 
        </div>
        <!-- 
